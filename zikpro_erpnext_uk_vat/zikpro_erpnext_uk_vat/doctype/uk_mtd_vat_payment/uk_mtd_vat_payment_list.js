frappe.listview_settings['UK MTD VAT Payment'] = {
    onload: function(listview) {
        ...
        // Add button to fetch
        listview.page.add_inner_button(__("Fetch Payment"), function () {
             open_payment_fetch_dialog(listview);
        }, __("Actions"));
    }
};

function open_payment_fetch_dialog(listview) {
    const dialog = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            ...
            frappe.call({
                method: "zikpro_erpnext_uk_vat.api.fetch_payments",
                args: {
                    from_date: values.from_date,
                    to_date: values.to_date
                },
                freeze: true,
                freeze_message: __("Fetching VAT Payments from HMRC..."),
                callback: function (r) {
                    if (!r.message) {
                    ...
                    listview.render();
                    frappe.show_alert({
                        message: __("Fetched {0} payments", [r.message.length]),
                        indicator: "green"
                    });
                }
            });
